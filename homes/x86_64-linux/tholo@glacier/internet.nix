      ...
      ''
        ...
        def dns-status [server?: string] {
          let result = (
            try {
              if ($server == null) {
                ^${pkgs.dnsutils}/bin/dig +time=2 +tries=1 +short $probe_domain A | complete
              } else {
                ^${pkgs.dnsutils}/bin/dig +time=2 +tries=1 +short @$server $probe_domain A | complete
              }
            } catch {
              { exit_code: 1, stdout: "", stderr: "" }
            }
        ...
        def render-status [status: record] {
          ...
          match $status.state {
            "up" => { print "signal: at least one upstream DNS server resolved the probe domain" },
            "down" => { print "signal: local router is reachable, but upstream DNS resolution failed" },
